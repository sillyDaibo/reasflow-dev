...
def test_canonicalize_root_level_publication_without_rewriting_content(tmp_path) -> None:
    (tmp_path / "survey.tex").write_text(
        r"survey source \input{survey_body}", encoding="utf-8"
    )
    (tmp_path / "survey_body.tex").write_text(
        r"body source \input{sections/body}", encoding="utf-8"
    )
    ...
    assert (tmp_path / "survey/survey.tex").read_text() == (
        r"survey source \input{survey_body}"
    )
    assert (tmp_path / "survey/survey_body.tex").read_text() == (
        r"body source \input{sections/body}"
    )
