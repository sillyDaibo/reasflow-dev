...
`batch_chat` maps to multiple `task` calls.
`chat` maps to one subagent call.
...
2. Prepare and write an outline:
   ...
   - dispatch `survey-outline` with the prepared prompt → `survey/outline.md`
...
4. Run one native long-form writing stage:
   - dispatch `survey-section-writer` with `survey/native_survey_prompt.json` → `survey/survey.md`
...
7. Run related-work synthesis:
   - dispatch `survey-related-works` with `related_works/rw_prompt.json` → `related_works/related_works.tex`
...
2. For each prompt: dispatch `survey-outline` via `task` → rough outlines
3. `autosurvey_tools.py merge-outline-data` → survey/stage2.json
4. dispatch `survey-outline` → survey/section_outline.md
5. `autosurvey_tools.py prepare-subsection-outline-data` → survey/stage3.json
6. For each prompt: dispatch `survey-outline` via `task` → subsection outlines
...
9. dispatch `survey-outline` → survey/outline.md

# Phase 2: Survey Writing
10. `autosurvey_tools.py prepare-subsection-data` → survey/stage5.json
11. For each section: dispatch `survey-section-writer` via `task`
12. For each section: citation-check prompts → dispatch `survey-section-writer` via `task`
...
16. For even-indexed subsections: dispatch `survey-section-writer` via `task`
17. `autosurvey_tools.py prepare-lce-data` (odd, updated content) → survey/lce_odd.json
18. For odd-indexed subsections: dispatch `survey-section-writer` via `task`
...
22. dispatch `survey-related-works` → related_works/related_works_raw.tex
...
26. For each criterion: dispatch `survey-judge` via `task`
